...
public class Rook extends Piece {
    public Rook(boolean isWhite, String img_file, String other) {
    ...
    public ArrayList<Square> getControlledSquares(Square[][] board, Square start) {
        ...
        for (int col = start.getCol() + 1; col < 8; col++) {
            if (board[start.getRow()][col].getOccupyingPiece() == null ||
                    (board[start.getRow()][col].getOccupyingPiece().getColor() != this.getColor())) {
        ...
        for (int row = start.getRow() + 1; row < 8; row++) {
            if ((board[row][start.getCol()].getOccupyingPiece() == null) ||
                    (board[row][start.getCol()].getOccupyingPiece().getColor() != this.getColor())) {
